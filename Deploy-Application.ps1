...
Try {
	...
	[string]$appVersion = '1808'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '3.7.0.1'
	[string]$appScriptDate = '06/27/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Set the initial Office folder paths
		[string] $dirOffice32 = Join-Path -Path "$envProgramFilesX86" -ChildPath 'Microsoft Office'
		[string] $dirOffice64 = Join-Path -Path "$envProgramFiles" -ChildPath 'Microsoft Office'
		[string] $dirOffice15C2R = Join-Path -Path "$envProgramFiles" -ChildPath 'Microsoft Office 15'
		...
		## Verify whether Office 2016 is already installed
			$officeVersion = Get-ItemProperty -LiteralPath 'HKLM:SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{90160000-0011-0000-0000-0000000FF1CE}' -ErrorAction 'SilentlyContinue' | Select-Object -ExpandProperty DisplayName


		## Show Welcome Message, close required apps, and verify there is enough disk space to complete the install
		Show-InstallationWelcome -PersistPrompt -CloseApps "excel,groove,iexplore,infopath,lync,onedrive,onenote,onenotem,outlook,mspub,powerpnt,winword,winproj,visio" -PromptToSave -CheckDiskSpace -RequiredDiskSpace 3000

			## Display Pre-Install cleanup status
			Show-InstallationProgress -StatusMessage "Uninstalling previous versions of Microsoft Office. This may take some time. Please wait..."

			# Remove any previous version of Office (if required)
			[string[]]$officeExecutables = 'excel.exe', 'groove.exe', 'infopath.exe', 'onenote.exe', 'outlook.exe', 'mspub.exe', 'powerpnt.exe', 'winword.exe', 'winproj.exe', 'visio.exe', 'officeclicktorun.exe'
			ForEach ($officeExecutable in $officeExecutables) {
				If (Test-Path -LiteralPath (Join-Path -Path $dirOffice32 -ChildPath "Office12\$officeExecutable") -PathType 'Leaf') {
					Write-Log -Message 'Microsoft Office 2007 was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
					Execute-Process -Path "$envSystem32Directory\cscript.exe" -Parameters "//B //Nologo `"$dirSupportFiles\OffScrub07.vbs`" ClientAll /S /Q /NoCancel" -WindowStyle "Hidden" -IgnoreExitCodes "1,2,3,16,42" -WaitForMsiExec
					Break
				}
			}
			ForEach ($officeExecutable in $officeExecutables) {
				If (Test-Path -LiteralPath (Join-Path -Path $dirOffice32 -ChildPath "Office14\$officeExecutable") -PathType 'Leaf') {
					Write-Log -Message 'Microsoft Office 2010 32-Bit was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
					Execute-Process -Path "$envSystem32Directory\cscript.exe" -Parameters "//B //Nologo `"$dirSupportFiles\OffScrub10.vbs`" ClientAll /S /Q /NoCancel" -WindowStyle "Hidden" -IgnoreExitCodes "1,2,3,16,42" -WaitForMsiExec
					Break
				}
			}
			ForEach ($officeExecutable in $officeExecutables) {
				If (Test-Path -LiteralPath (Join-Path -Path $dirOffice64 -ChildPath "Office14\$officeExecutable") -PathType 'Leaf') {
			...
			ForEach ($officeExecutable in $officeExecutables) {
				If (Test-Path -LiteralPath (Join-Path -Path $dirOffice32 -ChildPath "Office15\$officeExecutable") -PathType 'Leaf') {
					Write-Log -Message 'Microsoft Office 2013 MSI 32-Bit was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
					Execute-Process -Path "$envSystem32Directory\cscript.exe" -Parameters "//B //Nologo `"$dirSupportFiles\OffScrub_O15msi.vbs`" ClientAll /S /Q /NoCancel" -WindowStyle "Hidden" -IgnoreExitCodes "1,2,3,16,42" -WaitForMsiExec
					Break
				}
			}
			ForEach ($officeExecutable in $officeExecutables) {
				If (Test-Path -LiteralPath (Join-Path -Path $dirOffice64 -ChildPath "Office15\$officeExecutable") -PathType 'Leaf') {
					Write-Log -Message 'Microsoft Office 2013 MSI 64-Bit was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
					Execute-Process -Path "$envSystem32Directory\cscript.exe" -Parameters "//B //Nologo `"$dirSupportFiles\OffScrub_O15msi.vbs`" ClientAll /S /Q /NoCancel" -WindowStyle "Hidden" -IgnoreExitCodes "1,2,3,16,42" -WaitForMsiExec
					Break
				}
			}
			If ($officeVersion) {
				Write-Log -Message 'Microsoft Office 2016 MSI 32-Bit was detected. Will be uninstalled.' -Source $deployAppScriptFriendlyName
				Execute-Process -Path "$envSystem32Directory\cscript.exe" -Parameters "//B //Nologo `"$dirSupportFiles\OffScrub_O16msi.vbs`" ClientAll /S /Q /NoCancel" -WindowStyle "Hidden" -IgnoreExitCodes "1,2,3,16,42" -WaitForMsiExec
			}
		...
		$exitCode = Execute-Process -Path "$dirFiles\setup.exe" -Parameters "/configure configuration.xml" -WindowStyle "Hidden" -PassThru -WaitForMsiExec
        ...
        # Pin tiles to start menu. will only run from Software Center, not in Task Sequence (which will handle it in a later step in the sequence)
        If (-not $runningtasksequence)
         {
            #Gets OS Version, returns '10' for 10, and '6' for 7
            $oslongversion = (Get-ItemProperty -Path c:\windows\system32\hal.dll).VersionInfo.FileVersion
            $osversion = $oslongversion.split('.')[0]

            If($osversion -eq '10'){
             # Copy files to temp for scheduled task to run
             Copy-File -Path "$dirSupportFiles\PinIconsForUser.ps1" -Destination "C:\Temp\"
             Copy-File -Path "$dirSupportFiles\StartAndTaskbar365-2019.xml" -Destination "C:\Temp\"
             Execute-ProcessAsUser -Path "$PSHOME\powershell.exe" -Parameters "-Command & { & `"C:\Temp\PinIconsForUser.ps1`"; Exit `$LastExitCode }" -Wait
             Remove-File -Path "C:\Temp\PinIconsForUser.ps1"
             
             # Copy start layout to any account not yet on the machine (mostly for labs)
             Import-StartLayout -LayoutPath "C:\Temp\StartAndTaskbar365-2019.xml" -MountPath C:\
             Remove-File -Path "C:\Temp\StartAndTaskbar365-2019.xml"
             }
         }
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps "excel,groove,iexplore,infopath,lync,onedrive,onenote,onenotem,outlook,mspub,powerpnt,winword,winproj,visio"
		...
		Execute-Process -Path "$envSystem32Directory\cscript.exe" -Parameters "//B //Nologo `"$dirSupportFiles\OffScrubc2r.vbs`" ALL /Q /NoCancel" -WindowStyle "Hidden" -IgnoreExitCodes "1,2,3,16,42" -WaitForMsiExec
		$mainExitCode = "3010"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUgUoNE4aIDYmhLgB8bv5l/lAN
# QL6ggguhMIIFrjCCBJagAwIBAgIQBwNx0Q95WkBxmSuUB2Kb4jANBgkqhkiG9w0B
...
# gjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTRhMpZ7NyBuxJ5
# uJsTzret8GivSDANBgkqhkiG9w0BAQEFAASCAQCf23L13Jq1hpDkV4kJxPfiNP4d
# fH8GHrUnjs1/4d/XWGc8B008HIaEAfl2xP6Nc+95pSKwpNtNn0z00wpvLL/iRbM7
# nCcP9O0WBSd/FXe1ZXluV/ximFFSFClQEvUmLFYyEhyo0gahaiMJI89cxAk3OUO4
# e7cwfCEWidTO/x7IkBB92graOeYlIB6Uhm2CPC+Vst3sbLdGsa0uWGCEzO9cQSCC
# Wj+O3SlQBn4JaoBRhFmbW69mPapTdqdHleGpfoLY2MX0MWH5yI0NVSKJRF0HO27A
# W8blwdzu8AHXwWs1lWE04usVOlwlTgS/MV3St341GRXG5j2ar4mQIDDgGKYH
